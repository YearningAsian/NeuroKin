-- =====================================================================
-- NeuroKin — Snowflake DDL (fresh install)
-- =====================================================================
...
-- 1. Warehouse
CREATE WAREHOUSE IF NOT EXISTS NEUROKIN_WH
    WITH WAREHOUSE_SIZE = 'XSMALL'
    AUTO_SUSPEND = 60
    AUTO_RESUME  = TRUE;

-- 2. Database & Schema
CREATE DATABASE IF NOT EXISTS NEUROKIN_DB;
USE DATABASE NEUROKIN_DB;
CREATE SCHEMA IF NOT EXISTS PUBLIC;
USE SCHEMA PUBLIC;

-- 3. Roles & Grants (run as ACCOUNTADMIN; skip if already set up)
-- CREATE ROLE IF NOT EXISTS NEUROKIN_APP_ROLE;
-- GRANT USAGE ON DATABASE NEUROKIN_DB     TO ROLE NEUROKIN_APP_ROLE;
-- GRANT USAGE ON SCHEMA   NEUROKIN_DB.PUBLIC TO ROLE NEUROKIN_APP_ROLE;
-- GRANT ALL   ON ALL TABLES IN SCHEMA NEUROKIN_DB.PUBLIC TO ROLE NEUROKIN_APP_ROLE;
-- GRANT USAGE ON WAREHOUSE NEUROKIN_WH TO ROLE NEUROKIN_APP_ROLE;
-- CREATE USER IF NOT EXISTS NEUROKIN_USER PASSWORD='CHANGE_ME' DEFAULT_ROLE=NEUROKIN_APP_ROLE;
-- GRANT ROLE NEUROKIN_APP_ROLE TO USER NEUROKIN_USER;
